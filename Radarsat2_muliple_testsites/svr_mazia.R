...
set.seed(Sys.time())
trainIndices <- createDataPartition(features_Mazia$smc, p=0.8, list=FALSE, times=1)
training <- features_Mazia#[trainIndices,]
testing <- features_Mazia#[-trainIndices,]
#training <- CombDs
#testing <- CombDs
...
ctrl <- trainControl(method="cv", 
                     ...
                     allowParallel = T)
mod <- train(smc ~ hh + hv + lia + ndvi + height + slope + aspect,
             data=training,
             method='svmRadial',
             tuneLength = 5,
