...
#load SVR model
ANNdata <- readMat("X:/Workspaces/GrF/Processing/SCA_paper/results_emanuele/testset/ASCAT+SLOPE.mat")
...
rslist <- data.frame(true=ANNdata$smc[,1], est=ANNdata$smc.nn[,1], lc=as.factor(lclist))
uselc <- c(14, 20, 30, 40, 60, 90, 100, 110, 120, 130, 140, 150)

rslist <- rslist[lclist %in% uselc,]

#create correlation per land-cover output ----------------------------------------------------------
outdir <- "X:/Workspaces/GrF/Processing/SCA_paper/era_validation_ann/ASCAT_SLOPE/"
