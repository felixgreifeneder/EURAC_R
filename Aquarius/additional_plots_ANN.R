...
#load testset
#load("X:/Workspaces/GrF/Processing/Aquarius/Aquarius/parameters_global_R")
#load("X:/Workspaces/GrF/Processing/Aquarius/Aquarius_with_means/train_test_mean_slope.dat")

#load SVR model
ANNdata <- readMat("X:/Workspaces/GrF/Processing/SCA_paper/results_emanuele/testset/ASCAT+AQUARIUS.mat")


#retrieve land-cover
globcover <- raster("X:/Workspaces/GrF/01_Data/ANCILLARY/GLOBCOVER/GLOBCOVER_L4_200901_200912_V2.3.tif")

t <- matrix(c(ANNdata$lon[,1], ANNdata$lat[,1]), nrow(ANNdata$lat), 2)
lclist <- extract(globcover, t)
rslist <- data.frame(true=ANNdata$smc[,1], est=ANNdata$smc.nn[,1], lc=as.factor(lclist), lat=ANNdata$lat, lon=ANNdata$lon)
...
path <- paste(outdir, 'ann_rmse.png', sep="")
...
gridPlt + ggtitle("ANN prediction - RMSE\n")

dev.off()

path <- paste(outdir, 'ann_r.png', sep="")
...
gridPlt + ggtitle("ANN prediction - R\n")
